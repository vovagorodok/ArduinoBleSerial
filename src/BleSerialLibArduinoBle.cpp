...
BLECharacteristic rxCharacteristic(BLE_SERIAL_CHARACTERISTIC_UUID_RX, BLEWriteWithoutResponse,
                                   BLE_SERIAL_MAX_BUFFER_SIZE);
...
bool BleSerialLib::begin(const char* deviceName, BleSerialService& serialService) {
...
void BleSerialLib::onWrite(const BLECharacteristic& characteristic) {
    _serialService->handleData(rxCharacteristic.value(), rxCharacteristic.valueLength());
}
