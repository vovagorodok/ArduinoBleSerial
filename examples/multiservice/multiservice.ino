...
#include <ArduinoBleSerialMultiservice.h>
...
#ifdef BLE_SERIAL_LIB_NIM_BLE_ARDUINO
// clang-format off
...
void setup() {
    ...
    ArduinoBleSerial.begin(serialService);
    advertiseBle(NAME, BLE_SERIAL_SERVICE_UUID, LED_SERVICE_UUID);
#endif

#ifdef BLE_SERIAL_LIB_NIM_BLE_ARDUINO
    auto server = initBle(NAME);
    auto ledService = server->createService(LED_SERVICE_UUID);
    auto ledCharacteristic =
        ledService->createCharacteristic(LED_CHARACTERISTIC_UUID, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE);
    ledCharacteristic->setValue(false);
    ledCharacteristic->setCallbacks(&ledCallbacks);
    ledService->start();
    ArduinoBleSerial.begin(server, serialService);
    advertiseBle(server, NAME, BLE_SERIAL_SERVICE_UUID, LED_SERVICE_UUID);
#endif
}
